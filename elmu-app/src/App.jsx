...
const KnowledgeBase = ({ onClose, T, articles, setArticles }) => {
  ...
  const openRead = (a) => { setSelected({ ...a, views: a.views + 1 }); setArticles(prev => prev.map(x => x.id === a.id ? { ...x, views: x.views + 1 } : x)); setView("read"); };
    ...
    <Modal title="Knowledge Base" onClose={onClose} wide T={T}>
      <div style={{ display: "flex", gap: 10, marginBottom: 16, alignItems: "center" }}>
        <div style={{ position: "relative", flex: 1 }}>
          ...
          <Input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search articles by title, category, or content..." style={{ paddingLeft: 32 }} />
        </div>
    ...
    <Modal title="Knowledge Base" onClose={onClose} wide T={T}>
      ...
      <div style={{ background: T.card2, borderRadius: 10, padding: 24 }}>
        <div style={{ display: "flex", gap: 8, marginBottom: 12, alignItems: "center" }}>
          ...
          <span style={{ fontSize: 11, color: T.text3, marginLeft: "auto" }}>👁 {selected.views} views</span>
        </div>
  ...
  return (
    <Modal title={isEdit ? "Edit Article" : "New Article"} onClose={onClose} wide T={T}>
      ...
      <Field label="Article Title" required>
        <Input value={form.title} onChange={f("title")} placeholder="e.g. How to set up two-factor authentication" />
      </Field>
      <Field label="Category">
        <Select value={form.category} onChange={f("category")}>{KB_CATEGORIES.map(c => <option key={c}>{c}</option>)}</Select>
      </Field>
      <Field label="Content">
        <Textarea value={form.content} onChange={f("content")} placeholder={"Write your article here.\n\nTip: Use numbered steps for instructions, e.g.:\n1. First step\n2. Second step\n3. Third step"} style={{ minHeight: 240, fontFamily: "inherit", lineHeight: 1.7 }} />
      </Field>
      <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 8 }}>

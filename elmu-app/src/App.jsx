...
// Stable named components — defined once at module level to prevent remount-on-render
const TInput = ({ T, style: s, ...props }) => (
...
const TSelect = ({ T, children, style: s, ...props }) => (
...
const TTextarea = ({ T, style: s, ...props }) => (
      ...
      <div style={{ width: "100%", maxWidth: 440 }}>
        ...
        <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 14, padding: 28 }}>
          <Field label="Username">
            <TInput T={T} value={u} onChange={e => setU(e.target.value)} placeholder="e.g. carlos" onKeyDown={e => e.key === "Enter" && submit()} />
          </Field>
          <Field label="Password">
            <div style={{ position: "relative" }}>
              <TInput T={T} type={showPwd ? "text" : "password"} value={p} onChange={e => setP(e.target.value)} placeholder="••••••••" onKeyDown={e => e.key === "Enter" && submit()} style={{ paddingRight: 52 }} />
    ...
    <Modal title={`${ticket.id} — Ticket Details`} onClose={onClose} wide T={T}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 20 }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 20, alignItems: "start" }}>
          ...
          <div style={{ minWidth: 210 }}>
            <div style={{ background: T.card2, borderRadius: 8, padding: 14, marginBottom: 12 }}>
              ...
              <Field label="Status"><TSelect T={T} value={edit.status} onChange={e => setEdit({ ...edit, status: e.target.value })}>{STATUSES.map(s => <option key={s}>{s}</option>)}</TSelect></Field>
              <Field label="Assignee"><TSelect T={T} value={edit.assignee} onChange={e => setEdit({ ...edit, assignee: e.target.value })}><option>Unassigned</option>{agents.map(a => <option key={a.id}>{a.name}</option>)}</TSelect></Field>
              <Field label="Priority"><TSelect T={T} value={edit.priority} onChange={e => setEdit({ ...edit, priority: e.target.value })}>{PRIORITIES.map(p => <option key={p}>{p}</option>)}</TSelect></Field>
        ...
        <div>
          ...
          <TTextarea T={T} value={comment} onChange={e => setComment(e.target.value)} placeholder="Add a comment..." style={{ minHeight: 60, marginBottom: 8 }} />
...
const NewTicketModal = ({ onClose, onCreate, agents, T }) => {
  ...
  return (
    <Modal title="Create New Ticket" onClose={onClose} T={T}>
      <Field label="Title" required><TInput T={T} value={form.title} onChange={f("title")} placeholder="Brief description of the issue" /></Field>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        <Field label="Category"><TSelect T={T} value={form.category} onChange={f("category")}>{CATEGORIES.map(c => <option key={c}>{c}</option>)}</TSelect></Field>
        <Field label="Priority"><TSelect T={T} value={form.priority} onChange={f("priority")}>{PRIORITIES.map(p => <option key={p}>{p}</option>)}</TSelect></Field>
        <Field label="Requester" required><TInput T={T} value={form.requester} onChange={f("requester")} placeholder="Full name" /></Field>
        <Field label="Department"><TSelect T={T} value={form.dept} onChange={f("dept")}>{DEPARTMENTS.map(d => <option key={d}>{d}</option>)}</TSelect></Field>
        <Field label="Assignee"><TSelect T={T} value={form.assignee} onChange={f("assignee")}><option>Unassigned</option>{agents.map(a => <option key={a.id}>{a.name}</option>)}</TSelect></Field>
        <Field label="Tags"><TInput T={T} value={form.tags} onChange={f("tags")} placeholder="VPN, Windows, Printer" /></Field>
      </div>
      <Field label="Description"><TTextarea T={T} value={form.description} onChange={f("description")} placeholder="Steps to reproduce, error messages, etc." style={{ minHeight: 90 }} /></Field>
...
// ─── KB MODAL ─────────────────────────────────────────────────────────────────
const INITIAL_KB = [
  { id: "KB-001", title: "How to reset your Windows password", category: "Account", content: "1. Press Ctrl+Alt+Del and select 'Change a password'.\n2. Enter your old password, then your new password twice.\n3. Press the arrow to confirm.\n\nIf you're locked out, contact IT with your employee ID for a reset.", views: 342 },
  { id: "KB-002", title: "VPN Setup Guide – Windows & Mac", category: "Network", content: "Download the VPN client from the IT portal.\n\nWindows: Run the installer as Administrator, enter the server address vpn.elmu.local, and log in with your ELMU credentials.\n\nMac: Open the .pkg file and follow the setup wizard. Use the same server address and credentials.", views: 512 },
  { id: "KB-003", title: "Configure Outlook on a new device", category: "Software", content: "1. Open Outlook and click 'Add Account'.\n2. Enter your work email (you@elmu.local).\n3. Select 'Exchange' when prompted for account type.\n4. Enter server: mail.elmu.local\n5. Log in with your network password.", views: 289 },
  { id: "KB-004", title: "Printer troubleshooting steps", category: "Hardware", content: "1. Check that the printer is powered on and connected to the network.\n2. On Windows: Go to Settings > Printers & Scanners and remove then re-add the printer.\n3. Print a test page.\n4. If still offline, restart the print spooler service (services.msc > Print Spooler > Restart).\n5. Contact IT if the issue persists.", views: 198 },
  { id: "KB-005", title: "How to request new software access", category: "Access", content: "Submit a ticket under the 'Access' category with:\n- Software name and version\n- Business justification\n- Your manager's name for approval\n\nIT will process the request within 2 business days after manager approval.", views: 401 },
  { id: "KB-006", title: "MFA / 2FA Setup Instructions", category: "Security", content: "1. Download the Microsoft Authenticator app on your phone.\n2. Log in to portal.elmu.local and go to Security Settings.\n3. Click 'Set up two-factor authentication'.\n4. Scan the QR code with the Authenticator app.\n5. Enter the 6-digit code to verify setup.", views: 633 },
];

const KB_CATEGORIES = ["Account", "Network", "Software", "Hardware", "Access", "Security", "Other"];
const newKbId = (articles) => `KB-${String(articles.length + 1).padStart(3, "0")}`;

const KnowledgeBase = ({ onClose, T, articles, setArticles }) => {
  const [search, setSearch] = useState("");
  const [view, setView] = useState("list"); // "list" | "read" | "edit" | "new"
  ...
  const openNew = () => { setForm({ title: "", category: "Software", content: "" }); setView("new"); };
  const openEdit = (a) => { setForm({ title: a.title, category: a.category, content: a.content }); setSelected(a); setView("edit"); };
  const openRead = (a) => { setSelected(a); setView("read"); };

  const saveNew = () => {
    if (!form.title.trim()) return;
    const article = { id: newKbId(articles), ...form, views: 0 };
    ...
    <Modal title="Knowledge Base" onClose={onClose} wide T={T}>
      <div style={{ display: "flex", gap: 10, marginBottom: 16, alignItems: "center" }}>
        <div style={{ position: "relative", flex: 1 }}>
          ...
          <TInput T={T} value={search} onChange={e => setSearch(e.target.value)} placeholder="Search articles by title, category, or content..." style={{ paddingLeft: 32 }} />
        </div>
        <Btn onClick={openNew} icon="new">New Article</Btn>
      </div>
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 14 }}>
        {["All", ...KB_CATEGORIES].map(cat => (
          <button key={cat} onClick={() => setSearch(cat === "All" ? "" : cat)}
            style={{ padding: "4px 12px", borderRadius: 20, border: `1px solid ${T.border}`, background: (search === cat || (cat === "All" && !search)) ? "#4a9eff22" : T.card2, color: (search === cat || (cat === "All" && !search)) ? "#4a9eff" : T.text3, cursor: "pointer", fontSize: 12, fontWeight: 500 }}>
            {cat}
          </button>
        ))}
      </div>
      {filtered.length === 0 && (
        <div style={{ textAlign: "center", padding: "32px 0", color: T.text3 }}>
          ...
          No articles found. <button onClick={openNew} style={{ background: "none", border: "none", color: "#4a9eff", cursor: "pointer", fontSize: 13 }}>Create one?</button>
        </div>
      )}
      {filtered.map(a => (
        <div key={a.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "13px 16px", background: T.card2, borderRadius: 9, marginBottom: 8, borderLeft: "3px solid transparent", transition: "border-color 0.15s, background 0.1s" }}
          onMouseEnter={e => { e.currentTarget.style.borderLeftColor = "#4a9eff"; e.currentTarget.style.background = T.border; }}
          onMouseLeave={e => { e.currentTarget.style.borderLeftColor = "transparent"; e.currentTarget.style.background = T.card2; }}>
          <div style={{ flex: 1, cursor: "pointer" }} onClick={() => openRead(a)}>
            <div style={{ fontWeight: 600, color: T.text, marginBottom: 3 }}>{a.title}</div>
            <div style={{ fontSize: 12, color: T.text3, display: "flex", gap: 10 }}>
              <span style={{ background: "#4a9eff18", color: "#4a9eff", borderRadius: 4, padding: "1px 7px", fontWeight: 500 }}>{a.category}</span>
              <span>{a.id}</span>
            </div>
          </div>
          <div style={{ display: "flex", gap: 6, marginLeft: 12, flexShrink: 0 }}>
            <button onClick={() => openEdit(a)} style={{ background: "#4a9eff11", border: "1px solid #4a9eff33", borderRadius: 6, padding: "5px 10px", color: "#4a9eff", cursor: "pointer", fontSize: 11, fontWeight: 600, display: "flex", alignItems: "center", gap: 4 }}>
              <Icon name="edit" size={12} /> Edit
            </button>
            <button onClick={() => setDeleteConfirm(a)} style={{ background: "#ff3b3b11", border: "1px solid #ff3b3b33", borderRadius: 6, padding: "5px 10px", color: "#ff3b3b", cursor: "pointer", fontSize: 11, fontWeight: 600, display: "flex", alignItems: "center", gap: 4 }}>
      ...
      {deleteConfirm && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", zIndex: 1100, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
          <div style={{ background: T.modalBg, border: `1px solid ${T.border}`, borderRadius: 12, padding: 28, maxWidth: 380, width: "100%", textAlign: "center" }}>
            <div style={{ fontSize: 32, marginBottom: 12 }}>🗑️</div>
            <div style={{ fontWeight: 700, color: T.text, fontSize: 16, marginBottom: 8 }}>Delete Article?</div>
            <div style={{ fontSize: 13, color: T.text3, marginBottom: 20 }}>"{deleteConfirm.title}" will be permanently removed.</div>
            <div style={{ display: "flex", gap: 10, justifyContent: "center" }}>
              <Btn variant="ghost" onClick={() => setDeleteConfirm(null)}>Cancel</Btn>
              <Btn variant="danger" icon="delete" onClick={() => deleteArticle(deleteConfirm.id)}>Delete</Btn>
            </div>
          </div>
        </div>
      )}
  ...
  if (view === "read") return (
    <Modal title="Knowledge Base" onClose={onClose} wide T={T}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
        <button onClick={() => setView("list")} style={{ background: "none", border: "none", color: T.text3, cursor: "pointer", fontSize: 13, display: "flex", alignItems: "center", gap: 6 }}>
          ← Back to articles
        </button>
        <div style={{ display: "flex", gap: 8 }}>
          <Btn small icon="edit" onClick={() => openEdit(selected)}>Edit</Btn>
          <Btn small variant="danger" icon="delete" onClick={() => setDeleteConfirm(selected)}>Delete</Btn>
        </div>
      </div>
      <div style={{ background: T.card2, borderRadius: 10, padding: 24 }}>
        <div style={{ display: "flex", gap: 8, marginBottom: 12, alignItems: "center" }}>
          <span style={{ background: "#4a9eff18", color: "#4a9eff", borderRadius: 4, padding: "2px 10px", fontWeight: 600, fontSize: 12 }}>{selected.category}</span>
          <span style={{ fontSize: 11, color: T.text3 }}>{selected.id}</span>
        </div>
        <h2 style={{ margin: "0 0 20px", fontSize: 20, fontWeight: 800, color: T.text, lineHeight: 1.3 }}>{selected.title}</h2>
        <div style={{ fontSize: 14, color: T.text2, lineHeight: 1.8, whiteSpace: "pre-wrap", borderTop: `1px solid ${T.border}`, paddingTop: 16 }}>
          {selected.content || <em style={{ color: T.text3 }}>No content yet.</em>}
        </div>
      </div>
      {deleteConfirm && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", zIndex: 1100, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
  ...
  return (
    <Modal title={isEdit ? "Edit Article" : "New Article"} onClose={onClose} wide T={T}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
        <button onClick={() => setView(isEdit ? "read" : "list")} style={{ background: "none", border: "none", color: T.text3, cursor: "pointer", fontSize: 13 }}>← Back</button>
        <span style={{ fontSize: 15, fontWeight: 700, color: T.text }}>{isEdit ? `Editing ${selected.id}` : "Create New Article"}</span>
      </div>
      <Field label="Article Title" required>
        <TInput T={T} value={form.title} onChange={f("title")} placeholder="e.g. How to set up two-factor authentication" />
      </Field>
      <Field label="Category">
        <TSelect T={T} value={form.category} onChange={f("category")}>{KB_CATEGORIES.map(c => <option key={c}>{c}</option>)}</TSelect>
      </Field>
      <Field label="Content">
        <TTextarea T={T} value={form.content} onChange={f("content")} placeholder={"Write your article here.\n\nTip: Use numbered steps for instructions, e.g.:\n1. First step\n2. Second step\n3. Third step"} style={{ minHeight: 240, fontFamily: "inherit", lineHeight: 1.7 }} />
      </Field>
      <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 8 }}>
        <Btn variant="ghost" onClick={() => setView(isEdit ? "read" : "list")}>Cancel</Btn>
        <Btn icon={isEdit ? "check" : "new"} onClick={isEdit ? saveEdit : saveNew} disabled={!form.title.trim()}>
    ...
    <Modal title="Team Management" onClose={onClose} wide T={T}>
      ...
      {showAdd && (
        <div style={{ background: T.card2, borderRadius: 10, padding: 16, marginBottom: 16, border: `1px solid #4a9eff33` }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <Field label="Full Name" required><TInput T={T} value={form.name} onChange={f("name")} placeholder="e.g. Ahmad Faiz" /></Field>
            <Field label="Work Email" required><TInput T={T} value={form.email} onChange={f("email")} placeholder="faiz@elmu.local" /></Field>
            <Field label="Role"><TSelect T={T} value={form.role} onChange={f("role")}>{AGENT_ROLES.map(r => <option key={r}>{r}</option>)}</TSelect></Field>
            <Field label="Department"><TSelect T={T} value={form.dept} onChange={f("dept")}>{DEPARTMENTS.map(d => <option key={d}>{d}</option>)}</TSelect></Field>
            <Field label="Status"><TSelect T={T} value={form.status} onChange={f("status")}>{AGENT_STATUSES.map(s => <option key={s}>{s}</option>)}</TSelect></Field>
            <Field label="Username (login)"><TInput T={T} value={form.username} onChange={f("username")} placeholder="Auto: firstname if blank" /></Field>
            <Field label="Password (login)"><TInput T={T} value={form.password} onChange={f("password")} placeholder="Auto: name+1234 if blank" /></Field>
          </div>
    ...
    <Modal title="System Settings" onClose={onClose} wide T={T}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
        <div>
          ...
          <Field label="Org Name"><TInput T={T} value={vals.org} onChange={e => setVals({ ...vals, org: e.target.value })} /></Field>
          <Field label="IT Support Email"><TInput T={T} value={vals.email} onChange={e => setVals({ ...vals, email: e.target.value })} /></Field>
        </div>
        <div>
          <div style={{ fontSize: 11, fontWeight: 700, color: T.text3, marginBottom: 12, textTransform: "uppercase" }}>SLA Targets (hours)</div>
          <Field label="High"><TInput T={T} type="number" value={vals.slaHigh} onChange={e => setVals({ ...vals, slaHigh: e.target.value })} /></Field>
          <Field label="Medium"><TInput T={T} type="number" value={vals.slaMed} onChange={e => setVals({ ...vals, slaMed: e.target.value })} /></Field>
          <Field label="Low"><TInput T={T} type="number" value={vals.slaLow} onChange={e => setVals({ ...vals, slaLow: e.target.value })} /></Field>
        </div>
        <div>
...
// ─── ADMIN DASHBOARD ──────────────────────────────────────────────────────────
const AdminDashboard = ({ tickets, setTickets, agents, setAgents, kbArticles, setKbArticles, onLogout, darkMode, setDarkMode, T, currentUser }) => {
  ...
  return (
    <div style={{ minHeight: "100vh", background: T.bg, fontFamily: "'DM Sans','Segoe UI',sans-serif", color: T.text, display: "flex" }}>
      ...
      <div style={{ marginLeft: sidebarOpen ? 210 : 0, flex: 1, display: "flex", flexDirection: "column", transition: "margin-left 0.25s cubic-bezier(.4,0,.2,1)" }} className="admin-main">
        ...
        <div style={{ background: T.sidebar, borderBottom: `1px solid ${T.border}`, padding: "0 16px", height: 56, display: "flex", alignItems: "center", gap: 12, position: "sticky", top: 0, zIndex: 100 }}>
          ...
          <div style={{ position: "relative", flex: 1, maxWidth: 340 }}>
            <div style={{ position: "absolute", left: 10, top: "50%", transform: "translateY(-50%)", color: T.text3 }}><Icon name="search" size={14} /></div>
            <TInput T={T} value={search} onChange={e => setSearch(e.target.value)} placeholder="Search tickets..." style={{ paddingLeft: 32, fontSize: 12 }} />
          </div>
        ...
        <div style={{ padding: "16px" }}>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap", alignItems: "center" }}>
            ...
            {[["status", STATUSES, "Status"], ["priority", PRIORITIES, "Priority"], ["category", CATEGORIES, "Category"]].map(([key, opts, label]) => (
              <TSelect T={T} key={key} value={filters[key]} onChange={e => setFilters({ ...filters, [key]: e.target.value })} style={{ width: "auto", padding: "5px 8px", fontSize: 11 }}>
                <option value="">All {label}</option>
                {opts.map(o => <option key={o}>{o}</option>)}
              </TSelect>
            ))}
            <TSelect T={T} value={filters.assignee} onChange={e => setFilters({ ...filters, assignee: e.target.value })} style={{ width: "auto", padding: "5px 8px", fontSize: 11 }}>
      ...
      {activeModule === "kb" && <KnowledgeBase onClose={() => setActiveModule(null)} T={T} articles={kbArticles} setArticles={setKbArticles} />}
...
// ─── USER PORTAL ──────────────────────────────────────────────────────────────
const UserPortal = ({ onSubmit, onGoToLogin, myTickets, onUpdateTicket, darkMode, setDarkMode, T, kbArticles }) => {
    ...
    <div style={{ minHeight: "100vh", background: T.bg, fontFamily: "'DM Sans','Segoe UI',sans-serif", color: T.text }}>
      ...
      <div style={{ background: T.sidebar, borderBottom: `1px solid ${T.border}`, padding: "0 20px", height: 60, display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 100 }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          <button onClick={() => setView("kb")} style={{ background: "none", border: "none", color: view === "kb" ? "#3ecf8e" : T.text3, cursor: "pointer", fontSize: 13, fontWeight: 500, padding: "4px 8px" }}>Knowledge Base</button>
        ...
        <div style={{ maxWidth: 800, margin: "0 auto", padding: "40px 16px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14, marginBottom: 32 }} className="portal-cards">
            {[
              ...
              { icon: "kb", title: "Knowledge Base", desc: "Find answers to common issues", color: "#3ecf8e", action: () => setView("kb") },
            ].map(c => (
      ...
      {view === "submit" && (
        <div style={{ maxWidth: 600, margin: "0 auto", padding: "30px 16px" }}>
          ...
          <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 12, padding: 24 }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              <Field label="Full Name" required><TInput T={T} value={form.name} onChange={f("name")} placeholder="e.g. Ahmad Faiz" /></Field>
              <Field label="Work Email" required><TInput T={T} value={form.email} onChange={f("email")} placeholder="you@elmu.local" /></Field>
              <Field label="Department"><TSelect T={T} value={form.dept} onChange={f("dept")}><option value="">Select...</option>{DEPARTMENTS.map(d => <option key={d}>{d}</option>)}</TSelect></Field>
              <Field label="Category"><TSelect T={T} value={form.category} onChange={f("category")}>{CATEGORIES.map(c => <option key={c}>{c}</option>)}</TSelect></Field>
            </div>
            ...
            <Field label="Issue Title" required><TInput T={T} value={form.title} onChange={f("title")} placeholder="Brief description of your issue" /></Field>
            <Field label="Details"><TTextarea T={T} value={form.description} onChange={f("description")} placeholder="Error messages, steps tried, etc." style={{ minHeight: 100 }} /></Field>
        ...
        <div style={{ maxWidth: 800, margin: "0 auto", padding: "30px 16px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
            ...
            <div style={{ position: "relative", width: 240 }}>
              ...
              <TInput T={T} value={trackSearch} onChange={e => { setTrackSearch(e.target.value); setTrackedTicket(null); }} placeholder="Search tickets..." style={{ paddingLeft: 30, fontSize: 12 }} />
            </div>
          </div>
          ...
          {!trackedTicket ? (
            ...
            <div>
              ...
              <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 12, padding: 22 }}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <TInput T={T} value={userComment} onChange={e => setUserComment(e.target.value)} onKeyDown={e => e.key === "Enter" && sendUserComment()} placeholder="Reply to IT team..." style={{ flex: 1 }} />
      ...
      {/* KB — read-only view for users */}
      {view === "kb" && (
        <div style={{ maxWidth: 800, margin: "0 auto", padding: "30px 16px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
            <button onClick={() => { setView("home"); setKbSelected(null); setKbSearch(""); }} style={{ background: "none", border: "none", color: T.text3, cursor: "pointer", fontSize: 13 }}>← Back</button>
            <div style={{ fontWeight: 800, fontSize: 20, color: T.text, flex: 1 }}>Knowledge Base</div>
          </div>

          {kbSelected ? (
            <div>
              <button onClick={() => setKbSelected(null)} style={{ background: "none", border: "none", color: T.text3, cursor: "pointer", fontSize: 13, marginBottom: 16 }}>← All articles</button>
              <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 12, padding: 24 }}>
                <div style={{ display: "flex", gap: 8, marginBottom: 12, alignItems: "center" }}>
                  <span style={{ background: "#3ecf8e18", color: "#3ecf8e", borderRadius: 4, padding: "2px 10px", fontWeight: 600, fontSize: 12 }}>{kbSelected.category}</span>
                  <span style={{ fontSize: 11, color: T.text3 }}>{kbSelected.id}</span>
                </div>
                <h2 style={{ margin: "0 0 20px", fontSize: 19, fontWeight: 800, color: T.text, lineHeight: 1.3 }}>{kbSelected.title}</h2>
                <div style={{ fontSize: 14, color: T.text2, lineHeight: 1.85, whiteSpace: "pre-wrap", borderTop: `1px solid ${T.border}`, paddingTop: 16 }}>
                  {kbSelected.content || <em style={{ color: T.text3 }}>No content available.</em>}
                </div>
              </div>
            </div>
          ) : (
            <div>
              <div style={{ position: "relative", marginBottom: 16 }}>
                <div style={{ position: "absolute", left: 10, top: "50%", transform: "translateY(-50%)", color: T.text3 }}><Icon name="search" size={14} /></div>
                <input value={kbSearch} onChange={e => setKbSearch(e.target.value)} placeholder="Search articles by title, category or content..." style={{ width: "100%", background: T.inputBg, border: `1px solid ${T.border}`, borderRadius: 7, padding: "9px 12px 9px 32px", color: T.text, fontSize: 13, outline: "none", boxSizing: "border-box" }} />
              </div>
              {(kbArticles || []).filter(a => !kbSearch || a.title.toLowerCase().includes(kbSearch.toLowerCase()) || a.category.toLowerCase().includes(kbSearch.toLowerCase()) || (a.content || "").toLowerCase().includes(kbSearch.toLowerCase())).length === 0 && (
                <div style={{ textAlign: "center", padding: "40px 0", color: T.text3 }}>
                  <div style={{ fontSize: 32, marginBottom: 8 }}>📄</div>No articles found.
                </div>
              )}
              {(kbArticles || []).filter(a => !kbSearch || a.title.toLowerCase().includes(kbSearch.toLowerCase()) || a.category.toLowerCase().includes(kbSearch.toLowerCase()) || (a.content || "").toLowerCase().includes(kbSearch.toLowerCase())).map(a => (
                <div key={a.id} onClick={() => setKbSelected(a)}
                  style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 16px", background: T.card, border: `1px solid ${T.border}`, borderRadius: 10, marginBottom: 10, cursor: "pointer", borderLeft: "3px solid transparent", transition: "border-color 0.15s, transform 0.1s" }}
                  onMouseEnter={e => { e.currentTarget.style.borderLeftColor = "#3ecf8e"; e.currentTarget.style.transform = "translateX(3px)"; }}
                  onMouseLeave={e => { e.currentTarget.style.borderLeftColor = "transparent"; e.currentTarget.style.transform = "none"; }}>
                  <div>
                    <div style={{ fontWeight: 600, color: T.text, marginBottom: 4, fontSize: 14 }}>{a.title}</div>
                    <div style={{ fontSize: 12, color: T.text3, display: "flex", gap: 10 }}>
                      <span style={{ background: "#3ecf8e18", color: "#3ecf8e", borderRadius: 4, padding: "1px 7px", fontWeight: 500 }}>{a.category}</span>
                            </div>
                  </div>
                  <div style={{ color: T.text3, fontSize: 12, marginLeft: 12 }}>Read →</div>
                </div>
              ))}
            </div>
          )}
